...
def shrink_icon(icon: Image) -> Image:
  return ImageOps.expand(icon.resize((200, 200)), border=BORDER_WIDTH, fill=BORDER_COLOR)

def add_border(plate: Image) -> Image:
  return ImageOps.expand(plate, border=BORDER_WIDTH, fill=BORDER_COLOR)

def generate_name_plate(channel_name: str, person: Person, background_image: Image, font: str) -> Image:
  ...
  draw.multiline_text((365,300), f"@{person.username}\n{person.realname}", fill=(0,0,0), font=name_font)
...
def main():
  ...
  parser.add_argument("-t", "--template", default=BASE_IMAGE_PATH, help="path to template image of nameplate")
  parser.add_argument("-f", "--font", default=DEFAULT_FONT_PATH, help="path to font you want")
  parser.add_argument("-o", "--output", default=DEFAULT_OUTPUT_DIR, help="path to output")
  ...
  for attendee in attendee_list:
    plates.append(generate_name_plate(channel_name, attendee, copy.deepcopy(base_image), args.font))
  ...
  for ch in range(0, len(plates), COLS*ROWS):
    ...
    for y in range(ROWS):
      ...
      for i in range(COLS):
        try:
          output.paste(add_border(chunk[idx]), ((plate_width+BORDER_WIDTH)*i, h))
        except:
          output.paste(add_border(base_image), ((plate_width+BORDER_WIDTH)*i, h))
        finally:
          idx += 1
