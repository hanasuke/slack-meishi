...
class SlackClient:
  ...
  def get_channel_users(self, channel_name: str):
    try:
      ...
      return self.client.conversations_members(channel=channel_id, limit=1000)["members"]
  ...
  def get_user_info(self, user_id: str):
    ...
    return Person(username=user["profile"]["display_name"], realname=user["profile"]["real_name"], icon_url=user["profile"]["image_512"])

  def get_channel_id(self, channel_name: str):
    try:
      ch_list = self.client.conversations_list(limit=1000, types='public_channel')['channels']
      # TODO: adapt to cursor
